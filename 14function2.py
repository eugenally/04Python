...
def min_max(num):
  sum = 0
  # 매개변수를 통해 반복하게 되므로 컬랙션이라 예상할 수 있다.
  for val in num:
    # 각 요소를 누적해서 합산
    sum += val
...
'''
내부함수 : 파이썬에서는 함수 내부에 또다른 함수를 정의할 수 있다.
Java의 내부 클래스와 유사하다.
'''
print(f"{'내부함수':-^30}")
#  person외부함수
def person(name, age):
  ...
  def myName(n):
    print('이름:%s' % n)
  def myAge(a):
    return "나이:%s" % a # 리턴으로 불렀으니 print로 호출함
  # 내부함수 호출 1
  myName(name)
  # 내부함수 호출 2  print안에서 호출
  print(myAge(age))
# 외부함수를 호출하면 인수가 내부함수로 전달되어 결과가 출력됨
person('성유겸', 13)


# 함수에서 사용하는 가장 일반적인 매개 변수로 작성 순서대로 적용
print(f"{'매개변수1 : 순서매개변수':-^30}")

def paramFunc1(str1, str2):
  print(str1, str2)
  return
cont = "은 매우 좋은 프로그램 입니다."
paramFunc1('Python', cont)

# 매개변수명을 그대로 사용하여 값을 전달, 따라서 순서없이 작성 가능.
print(f"{'매개변수2 : 키워드 매개변수'}")
...
# 매개변수로 값이 전달되지 않으면 디폴트 값을 사용하는 방식
print(f"{'매기변수3 : 디폴트 매개변수':-^30}")
...
#  **man은 딕셔녀리 형태로 인수를 전달받아 사용한다.
print(f"{'매개변수4 : 가변 매개변수(딕셔너리)':-^30}")

def paramFunc5(**man):
  print('**man', man)
  #딕셔너리 이므로 key와 Value를 구분해서 사용할 수 잇다
  for key in man:
    print(key, "=>",man[key])
