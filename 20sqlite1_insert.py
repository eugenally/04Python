# SQLite 모듈은 기본으로 제공되므로 import 만 하면 사용가능
import sqlite3
import random
...
try:
  '''
  ㅌㅔ이블이 없는 최초의 실행이라면 새롭게 생성된다. 컬럼의 자려형은 파이선의 자료형
  을 사요했는데, SQLite의 자료형을 사욯해도 오디낟.
  '''
  ...
  '''
  2번째 실행에서 테이블이 이미 생성된 상태라면 예외가 발생하게 되므로
  except절에서 처리해야한다. 혹은 쿼리문에 if not exist를 
  추가해도 된다.
  '''
...
# 레코드 2개 이상 삽입. 튜플로 구성된 리스트 사용함, 이때는  executemany()함수를 사용한다.
curs.executemany('insert into people values (?,?,?)',
                 [(f'강감찬{rnd}','장군',700),
                  (f'홍길동{rnd}','의적',00)])

# 반복문for문을 이용한 여러개 데이터 삽입
