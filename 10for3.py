...
#서기 1년 1월 1일(월요일) 부터 입력한 날짜까지의 누적일수를 누적
total_days = 0
'''
월별 날짜수를 리스트로 정의. 
1월을 인덱스 1로 사용하기위해 요소는 0으로 설정한다.
'''
...
for d in range(1,year):
  if d% 400 ==0:# 400으로 나눠지면 윤년
    ...
    total_days =total_days + 366
  else:# 그외는 모두 평년이므로 365일로 계산
    total_days =total_days +365
    
'''
입력 년도의 각 월의 날짜수를 누적해서 합산 한다. 
만약 1월을 입력햇다면 이 For문은 실행 되지 않는다.
'''
...
'''
입력월이 3 이상이고 입력년도가 윤년인 경우 1을 더해줘야 한다
윤년은 2월 29일 까지 있기 때문이다.
만약 1월 혹은 2월을 입력했다면 이부분은 고려할 필요가 없다.
'''
...
'''
서기 1년 1월 1일 은 월요일 이므로 7로 나눈 나머지를 통해 요일응 판단 할 수 있다.
나누어 떨어지면 일요일 ,1이 남으면 월요일로 판단 한다.
'''
