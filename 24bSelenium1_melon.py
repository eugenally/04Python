...
# 크롬 로드. 이때 웹 브라우저가 실행된다.
driver = webdriver.Chrome()

'''
셀레니움 크롬 브라우저를 이용해서 크롤링 할 페이지를 띄운후
데이터를 얻어온다. 이때 비동기 통신을 통해 데이터 로드하는 경우 조금 늦게
로딩되는 경우가 있으므로 셀레니움에서는 '암묵적대기'가 필요한 경우가 있다.
이런경우 5초까지 대기하겠다는 선언이다.
'''

driver.implicitly_wait(5)

'''
time모듈을 사용하는 경우에는 로딩과 상관없이 모조건 5초를 대기한다.
...
# 뷰티풀 숲을 임포트 한후 얻어온 데이터를 Soup객체로 반환
...
# 엑셀로 저장
pd_data.to_excel('./saveFiles/melon_chart2.xlsx', index=False)
